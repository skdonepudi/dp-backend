...
export const getMarkets = async (token: string) => {
  ...
  console.log(
    `These are the latest market outcomes of **${getKeyByValue(
      TOKENLIST,
      token
    )}**`
  );
  ...
  if (marketsResponse.success && marketsResponse.data?.markets?.length) {
    ...
    const marketsWithOutcomes = marketsResponse.data.markets
      .filter((market) => market.account.marketOutcomesCount > 0)
      .filter(
        (market) => market.account.marketLockTimestamp.toNumber() > currentTime
      );
